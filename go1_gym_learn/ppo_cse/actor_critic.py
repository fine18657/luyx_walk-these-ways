...
class AC_Args(PrefixProto, cli=False):
    # policy
    init_noise_std = 1.0    #初始动作噪声的标准差
    actor_hidden_dims = [512, 256, 128]     #Actor 网络的隐藏层维度
    critic_hidden_dims = [512, 256, 128]    #Critic 网络的隐藏层维度
    activation = 'elu'  # can be elu, relu, selu, crelu, lrelu, tanh, sigmoid   激活函数的选择

    adaptation_module_branch_hidden_dims = [256, 128]   #adaptation_module_branch_hidden_dims
...
class ActorCritic(nn.Module):
    ...
    def __init__(self, num_obs,
        ...
        # Value function  价值网络，输入为s，输出为v,1个数据，表示价值
        critic_layers = []
        ...
        # Action noise
        self.std = nn.Parameter(AC_Args.init_noise_std * torch.ones(num_actions))   #动作噪声的标准差
    ...
    def act(self, observation_history, **kwargs):
        self.update_distribution(observation_history)   #使用一定的噪声，模拟真实情况。更新正态分布
        return self.distribution.sample()   #从上述正态分布中随机采样（例：采样很多次，则这些样本的均值和方差满足上述正态分布）
