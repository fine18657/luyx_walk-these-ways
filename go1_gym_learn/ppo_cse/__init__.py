...
class RunnerArgs(PrefixProto, cli=False):
    ...
    max_iterations = 1500  # number of policy updates   策略更新的迭代次数

    # logging
    save_interval = 400  # check for potential saves every this many iterations 每隔多少次迭代保存一次
    save_video_interval = 100   # 每隔多少次迭代保存视频
    log_freq = 20    # 日志记录的频率

    # load and resume
    resume = False  # 是否从上次的保存点继续训练
    load_run = -1  # -1 = last run   需要加载的运行编号，-1 表示最后一次运行
    checkpoint = -1  # -1 = last saved model    需要加载的检查点，-1 表示最后保存的模型
    resume_path = None  # updated from load_run and chkpt   从 load_run 和 checkpoint 更新的路径
    resume_curriculum = True    #是否恢复课程学习的状态
...
class Runner:
    ...
    def __init__(self, env, device='cpu'):
        ...
        if RunnerArgs.resume:
            ...
            if hasattr(self.env, "curricula") and RunnerArgs.resume_curriculum:
                # load curriculum state 恢复课程的权重分布
        ...
        self.tot_timesteps = 0  #总的时间步数
        self.tot_time = 0   #总的训练时间
        self.current_learning_iteration = 0 #当前的训练迭代次数
        self.last_recording_it = 0  #上次记录的迭代次数
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False, eval_freq=100, curriculum_dump_freq=500, eval_expert=False):
        #num_learning_iterations: 指定学习迭代的次数     init_at_random_ep_len: 是否随机初始化每个环境的初始长度
        #eval_freq: 指定评估的频率，每多少次迭代进行一次评估        curriculum_dump_freq: 指定课程学习状态的保存频率     eval_expert: 是否在评估阶段使用专家模型
        from ml_logger import logger
        # initialize writer
