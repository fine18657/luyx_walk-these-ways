...
class Cfg(PrefixProto, cli=False):
    ...
    class terrain(PrefixProto, cli=False):
        mesh_type = 'trimesh'  # "heightfield" # none（没有地形）, plane（平面地形）, heightfield（高度场地形） or trimesh（三角网格地形）
    ...
    class reward_scales(ParamsProto, cli=False):
        termination = -0.0
        tracking_lin_vel = 1    #1.0
        tracking_ang_vel = 0.5    #0.5
        lin_vel_z = 0     #-0.02  对机器人在z轴（垂直方向）上的线速度进行惩罚
        ang_vel_xy = 0    #-0.001     对机器人在xy平面上的角速度进行惩罚
        orientation = 0.0     #重力向下分量
        orientation_control = -5.0  # -5.0    让机器人的姿态符合期望的滚转和俯仰角
        torques = 0     #-0.00001
        dof_vel = 0   #-1e-4  避免过高的速度
        dof_acc = 0     #-2.5e-7    todo
        base_height = -0.
        feet_air_time = 0   #1.0
        collision = 0     #-5.0   根据碰撞次数计算惩罚
        feet_stumble = -0.0
        action_rate = 0     #-0.01   todo
        stand_still = -0.
        tracking_lin_vel_lat = 0.
        tracking_lin_vel_long = 0.
        tracking_contacts = 0.
        tracking_contacts_shaped = 0.
        tracking_contacts_shaped_force = 4    #4.0    足端期望接触力
        tracking_contacts_shaped_vel = 4      #4.0    足端期望接触速度
        jump = 10.0  #10.0   #鼓励机器人达到预定的高度
        energy = 0.0
        energy_expenditure = 0.0
        survival = 0.0
        dof_pos_limits = 0.0
        feet_contact_forces = 0.
        feet_slip = 0     #-0.04   避免脚部打滑   todo
        feet_clearance_cmd_linear = 0     #-30.0    旨在让机器人达到预期的脚步抬高高度
        dof_pos = -0.0    #保持关节在合理范围内
        action_smoothness_1 = -0.1   #-0.1   动作平滑
        action_smoothness_2 = 0   #-0.1   动作平滑

        base_motion = 0.
        feet_impact_vel = 0.0    #惩罚脚部与地面接触时的过高速度
        raibert_heuristic = -10     #-10.0  启发式算法

    class normalization(PrefixProto, cli=False):
        clip_observations = 100.    #动作裁剪的范围，用于限制动作值的最大和最小值
        clip_actions = 100.
