...
class LeggedRobot(BaseTask):
    ...
    def step(self, actions):
        ...
        """
        clip_actions = self.cfg.normalization.clip_actions  #动作裁剪的范围，用于限制动作值的最大和最小值
        self.actions = torch.clip(actions, -clip_actions, clip_actions).to(self.device) #裁剪动作张量的值
        # step physics and render each frame
        #保存前一时刻的状态：基础位置（x，y，z）、四元数、线速度、足部速度
        self.prev_base_pos = self.base_pos.clone()
        self.prev_base_quat = self.base_quat.clone()
        self.prev_base_lin_vel = self.base_lin_vel.clone()
        self.prev_foot_velocities = self.foot_velocities.clone()
        self.render_gui()
        #指定控制解耦率  每个动作应用多少次物理模拟。用于在一次物理步长内多次应用相同的控制信号，以提高控制稳定性
        for _ in range(self.cfg.control.decimation):
            self.torques = self._compute_torques(self.actions).view(self.torques.shape)
            #将计算出的力矩应用到模拟环境中的每个关节上
            self.gym.set_dof_actuation_force_tensor(self.sim, gymtorch.unwrap_tensor(self.torques))
            self.gym.simulate(self.sim)
            # if self.device == 'cpu':
            self.gym.fetch_results(self.sim, True)  #获取仿真结果
            self.gym.refresh_dof_state_tensor(self.sim) #刷新机器人的关节状态张量，以反映最新的物理模拟结果
    ...
    def post_physics_step(self):
        ...
        self.reset_idx(env_ids)     #对需要重置的环境（由 reset_buf 标记）执行重置操作
    ...
    def check_termination(self):
        ...
        if self.cfg.rewards.use_terminal_body_height:   #false  身体高度不足时重置环境
    ...
    def reset_idx(self, env_ids):
        ...
        self._call_train_eval(self._randomize_dof_props, env_ids)   #随机化 DOF（自由度）
        if self.cfg.domain_rand.randomize_rigids_after_start:   #开始后随机化刚体属性（true）
        ...
        # fill extras
        train_env_ids = env_ids[env_ids < self.num_train_envs]      #num_train_envs：2048
        if len(train_env_ids) > 0:
            self.extras["train/episode"] = {}
            #记录了每个训练环境在当前episode中累积的奖励，保存并重置这些累积的奖励，为下一次训练做准备。
        ...
        eval_env_ids = env_ids[env_ids >= self.num_train_envs]      #空
    ...
    def set_idx_pose(self, env_ids, dof_pos, base_state):       #未被使用
        #设置给定环境（env_ids）中某些机器人或物体的关节位置（dof_pos）和根部状态
        if len(env_ids) == 0:
            return
    ...
    def compute_reward(self):
        ...
        #计算回报
        self.rew_buf[:] = 0.    #总奖励
        self.rew_buf_pos[:] = 0.    #正奖励
        self.rew_buf_neg[:] = 0.    #负奖励
        for i in range(len(self.reward_functions)):
            ...
            self.episode_sums[name] += rew  #将该奖励函数的结果累加到 episode_sums 中
            if name in ['tracking_contacts_shaped_force', 'tracking_contacts_shaped_vel']:  #特殊处理某些奖励函数
                self.command_sums[name] += self.reward_scales[name] + rew
            else:
                self.command_sums[name] += rew
        if self.cfg.rewards.only_positive_rewards:  #false
            self.rew_buf[:] = torch.clip(self.rew_buf[:], min=0.)
        elif self.cfg.rewards.only_positive_rewards_ji22_style: #TODO: update   true
            #一个乘积，左边为正奖励，右边为负奖励的指数（sigma_rew_neg=0.02），其结果为大于0小于1的数。
            #详细见论文
    ...
    def compute_observations(self):
        ...
        #重力（3），命令（15）关节位置与默认位置之差（12），关节速度与默认速度之差（12），动作（12）
        ...
        # add noise if needed
        if self.add_noise:
            #torch.rand_like生成obs_buf 形状相同的张量，每个值在0，1范围内。乘以2-1，每个值在[-1，1]，加上该值模拟噪声
        ...
        if self.cfg.env.priv_observe_friction:  #true   机器人各部件的摩擦系数
            ...
            self.next_privileged_obs_buf = torch.cat((self.next_privileged_obs_buf,
                                                      (self.friction_coeffs[:, 0].unsqueeze(
                                                          1) - friction_coeffs_shift) * friction_coeffs_scale),
                                                     dim=1)
        if self.cfg.env.priv_observe_ground_friction:   #false  地面的摩擦系数
            ...
            self.next_privileged_obs_buf = torch.cat((self.next_privileged_obs_buf,
                                                      (self.ground_friction_coeffs.unsqueeze(
                                                          1) - friction_coeffs_shift) * friction_coeffs_scale),
                                                     dim=1)
        if self.cfg.env.priv_observe_restitution:   #true   物体碰撞时的恢复系数
            ...
            self.next_privileged_obs_buf = torch.cat((self.next_privileged_obs_buf,
                                                      (self.restitutions[:, 0].unsqueeze(
                                                          1) - restitutions_shift) * restitutions_scale),
                                                     dim=1)
        if self.cfg.env.priv_observe_base_mass: #false  机器人质量
            ...
            self.next_privileged_obs_buf = torch.cat((self.next_privileged_obs_buf,
                                                      (self.payloads.unsqueeze(1) - payloads_shift) * payloads_scale),
                                                     dim=1)
        if self.cfg.env.priv_observe_com_displacement:  #false  质心偏移量
            ...
            self.next_privileged_obs_buf = torch.cat((self.next_privileged_obs_buf,
                                                      (
                                                              self.com_displacements - com_displacements_shift) * com_displacements_scale),
                                                     dim=1)
        if self.cfg.env.priv_observe_motor_strength:    #false  电机强度
            ...
            self.next_privileged_obs_buf = torch.cat((self.next_privileged_obs_buf,
                                                      (
                                                              self.motor_strengths - motor_strengths_shift) * motor_strengths_scale),
                                                     dim=1)
        if self.cfg.env.priv_observe_motor_offset:  #false  电机偏移
            ...
            self.next_privileged_obs_buf = torch.cat((self.privileged_obs_buf,
                                                      (
                                                              self.motor_offsets - motor_offset_shift) * motor_offset_scale),
                                                     dim=1)
        if self.cfg.env.priv_observe_body_height:   #false  身体高度
            ...
            self.next_privileged_obs_buf = torch.cat((self.next_privileged_obs_buf,
                                                      ((self.root_states[:self.num_envs, 2]).view(
                                                          self.num_envs, -1) - body_height_shift) * body_height_scale),
                                                     dim=1)
        if self.cfg.env.priv_observe_body_velocity: #false  身体线速度
            ...
            self.next_privileged_obs_buf = torch.cat((self.next_privileged_obs_buf,
                                                      ((self.base_lin_vel).view(self.num_envs,
                                                                                -1) - body_velocity_shift) * body_velocity_scale),
                                                     dim=1)
        if self.cfg.env.priv_observe_gravity:   #false  重力
        ...
        if self.cfg.env.priv_observe_clock_inputs:  #false  步态信息
            self.privileged_obs_buf = torch.cat((self.privileged_obs_buf,
                                                 self.clock_inputs), dim=-1)

        if self.cfg.env.priv_observe_desired_contact_states:    #false  接触状态
            self.privileged_obs_buf = torch.cat((self.privileged_obs_buf,
                                                 self.desired_contact_states), dim=-1)
    ...
    def create_sim(self):
        #初始化和设置模拟环境，根据配置的地形类型创建合适的地形，然后创建整体的模拟环境。
        """ Creates simulation, terrain and evironments
        """
        self.up_axis_idx = 2  # 2 for z, 1 for y -> adapt gravity accordingly   z为上轴
        self.sim = self.gym.create_sim(self.sim_device_id, self.graphics_device_id, self.physics_engine,
                                       self.sim_params)

        mesh_type = self.cfg.terrain.mesh_type  #trimesh
        if mesh_type in ['heightfield', 'trimesh']:
            if self.eval_cfg is not None:   #eval_cfg：None
    ...
    def _call_train_eval(self, func, env_ids):
        ...
        if len(env_ids_eval) > 0:
            ...
            if ret is not None and ret_eval is not None: ret = torch.cat((ret, ret_eval), axis=-1)  #沿着最后一个维度拼接在一起

        return ret

    def _randomize_gravity(self, external_force = None):
        #随机化或设置模拟环境中的重力
        if external_force is not None:  #是否提供外部力
    ...
    def _post_physics_step_callback(self):
        #该方法在每个物理步骤后调用，执行一系列操作，如机器人位置重定位、命令重新采样、地形高度测量、随机推动机器人
    ...
    def _resample_commands(self, env_ids):
        #对特定环境（env_ids）重新采样指令       env_ids：需要重新采样的环境ID
        if len(env_ids) == 0: return

        # resampling_time:重新采样指令的时间间隔，决定了多久对环境的指令进行一次重新采样
        #timesteps 是重新采样过程中所需的时间步数（steps）
        #ep_len 计算每个环境的最大步数
        ...
        #基于环境的表现和特定任务的奖励值，更新课程学习（curricula）
        #category=pronk、trot、pace、bound
        for i, (category, curriculum) in enumerate(zip(self.category_names, self.curricula)):
            #通过比较 self.env_command_categories 中的值与 i（当前类别索引），找到属于当前类别的环境 ID
            #i，category取值：0-pronk，1-trot，2-pace，3-bound
    ...
    def _compute_torques(self, actions):
        ...
        if self.cfg.domain_rand.randomize_lag_timesteps:    #模拟滞后，使用前几步的动作
    ...
    def _teleport_robots(self, env_ids, cfg):
        ...
        """
        #将机器人从模拟环境的边缘重新定位到环境的另一边，以防止它们掉出边界
        if cfg.terrain.teleport_robots:     #todo false  ？？
            thresh = cfg.terrain.teleport_thresh    #机器人被视为“太接近边缘”的阈值

            x_offset = int(cfg.terrain.x_offset * cfg.terrain.horizontal_scale)     #在X轴方向上的偏移量，用于调整位置
    ...
    def _init_custom_buffers__(self):
        # domain randomization properties   初始化了用于域随机化和模拟控制的各种缓冲区   todo 域随机化是什么？结合论文
